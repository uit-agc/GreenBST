...
do 
	...
	for UPD in 10 50
	do
		./${bin}.energy.${ORDER} -t ${ORDER} -n $thrd -u $UPD -i $init -r $rang >> $bin.order.stdmalloc.$stamp 2>&1
		LD_PRELOAD=$jemalloc ./${bin}.energy.${ORDER} -t ${ORDER} -n $thrd -u $UPD -i $init -r $rang >> $bin.order.jemalloc.$stamp 2>&1
	done
done
